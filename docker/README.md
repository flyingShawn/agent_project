...
| 聊天历史 | Docker volume `pg_data` | PostgreSQL 数据库（会话、消息、运维简报、任务执行、知识库条目等） |
...
> **注意**：Docker volume 的完整名称取决于 `docker-compose.yml` 所在目录名，格式为 `{目录名}_qdrant_data` 和 `{目录名}_pg_data`。可通过 `docker volume ls` 查看实际名称。以下命令中使用 `COMPOSE_PROJECT` 变量代替，请根据实际情况替换。
...
# 备份 Qdrant volume（替换 COMPOSE_PROJECT 为实际前缀）
docker run --rm -v COMPOSE_PROJECT_qdrant_data:/data -v $(pwd):/backup alpine tar czf /backup/qdrant_backup.tar.gz /data

# 恢复备份
docker run --rm -v COMPOSE_PROJECT_qdrant_data:/data -v $(pwd):/backup alpine tar xzf /backup/qdrant_backup.tar.gz -C /
```
...
# 备份 PostgreSQL volume（替换 COMPOSE_PROJECT 为实际前缀）
docker run --rm -v COMPOSE_PROJECT_pg_data:/data -v $(pwd):/backup alpine tar czf /backup/pg_data_backup.tar.gz /data

# 恢复备份
docker run --rm -v COMPOSE_PROJECT_pg_data:/data -v $(pwd):/backup alpine tar xzf /backup/pg_data_backup.tar.gz -C /
```
...
| 项目 | 最低要求 | 建议配置 |
|------|---------|---------|
| 操作系统 | Linux（CentOS 7+/Ubuntu 18.04+）或 Windows Server 2019+ | Ubuntu 20.04+ |
...
> **注意**：Docker 镜像与 CPU 架构绑定。打包机器和部署机器的 CPU 架构必须一致。
...
> **docling-sync 镜像**：打包脚本会自动检测本地是否已构建 docling-sync 镜像。如果已构建则自动包含，未构建则跳过并提示。
...
部署脚本会自动完成：
1. 校验镜像文件完整性（如存在校验文件）
2. 从 tar 文件加载所有 Docker 镜像（无需联网），加载失败会终止
3. 从 `.env.example` 创建 `.env` 配置文件
4. 创建数据目录（`data/desk-agent/docs` 等）
5. 复制 nginx.conf 和 entrypoint 脚本到工作目录
6. 启动所有服务
...
# CORS 跨域配置（必须包含前端实际访问地址）
CORS_ORIGINS=http://目标机器IP:81,http://localhost:81

# 其他按需修改...
```

> **重要**：`CORS_ORIGINS` 必须包含前端实际访问地址，否则前端页面无法调用后端 API。
...
| 端口 | 协议 | 服务 | 方向 | 说明 |
|------|------|------|------|------|
| 81 | TCP | frontend | 入站 | 前端页面访问 |
| 8000 | TCP | backend | 入站 | 后端 API |
| 5432 | TCP | postgres | 仅容器内 | PostgreSQL（不应对外暴露） |
| 6333 | TCP | qdrant | 仅容器内 | Qdrant REST API |
| 6334 | TCP | qdrant | 仅容器内 | Qdrant gRPC |
| 11434 | TCP | ollama | 出站 | Ollama API |
| 3306 | TCP | MySQL | 出站 | 业务数据库 |

### 12.4 离线部署升级流程

1. **有网机器**：拉取最新代码，重新构建镜像，运行打包脚本
2. **内网机器**：
   ```bash
   # 仅加载新镜像（不停止服务）
...
Docker 镜像与 CPU 架构绑定。打包和部署机器的 CPU 架构必须一致。如需跨架构构建，使用 `docker buildx build --platform linux/amd64`。

**Q: 如何更新离线包？**

参考 [12.4 离线部署升级流程](#124-离线部署升级流程)。
