...
export async function listOpsReports({ limit = 20, unreadOnly = false } = {}) {
  const response = await fetch(`${API_BASE}/reports?limit=${limit}&unread_only=${unreadOnly}`)
...
export async function markOpsReportRead(reportId) {
  const response = await fetch(`${API_BASE}/reports/${reportId}/read`, {
    method: 'PUT',
  })
  return parseJsonResponse(response)
}
...
export async function runOpsReportNow() {
  const response = await fetch(`${API_BASE}/reports/run`, {
    method: 'POST',
  })
  return parseJsonResponse(response)
}
