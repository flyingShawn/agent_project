...
| Agent 工具 | `agent_backend/agent/tools/` | @tool 装饰器定义的工具（当前 10 个） |
...
| 聊天历史 | `agent_backend/db/` | chat_history.py（SQLite异步引擎）+ models.py（ORM模型：Conversation/Message/AgentTask/AgentTaskResult） |
| 定时任务 | `agent_backend/scheduler/` | manager.py（APScheduler封装）+ executor.py（任务执行器） |
...
| UI 组件 | `agent_frontend/src/components/` | Vue 组件（ChatBox/Sidebar/MessageBubble/ChartBlock/ImageUploader） |
| 组合函数 | `agent_frontend/src/composables/` | Vue Composables（useConversations） |
...
- **配置文件**：小写+下划线/连字符（`schema_metadata.yaml`、`scheduled_tasks.yaml`）
...
| 在 `db/` 下放业务数据库连接 | 放 `sql_agent/` | `db/` 只管聊天历史和任务持久化，业务数据库是 SQL 功能的一部分 |
...
| 在 `agent/` 下放调度器代码 | 放 `scheduler/` | 调度器是独立模块，不属于 Agent 编排 |
| 在 `db/` 下放调度器逻辑 | 放 `scheduler/` | `db/` 只管 ORM 模型和引擎，调度逻辑在 `scheduler/` |
